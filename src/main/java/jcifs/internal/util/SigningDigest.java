...
public class SigningDigest {
    ...
    public void sign ( byte[] data, int offset, int length, ServerMessageBlock request, ServerMessageBlock response ) {
        ...
        request.setSignSeq(this.signSequence);
        if ( response != null ) {
            response.setSignSeq(this.signSequence + 1);
            response.setVerifyFailed(false);
        }
     ...
     */
    public boolean verify ( byte[] data, int offset, ServerMessageBlock msg ) {
        ...
        if ( ( msg.getFlags2() & SmbConstants.FLAGS2_SECURITY_SIGNATURES ) == 0 ) {
            ...
            return msg.setVerifyFailed(false);
        }
        ...
        for ( int i = 0; i < 8; i++ ) {
            if ( signature[ i ] != data[ offset + SmbConstants.SIGNATURE_OFFSET + i ] ) {
                ...
                return msg.setVerifyFailed(true);
            }
        }
